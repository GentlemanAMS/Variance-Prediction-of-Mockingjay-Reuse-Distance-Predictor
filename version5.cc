...
#if ETR_UPDATE
        int64_t ETRerror[LLC_SETS][LLC_WAYS];
...
#if ETR_UPDATE
    for(int w = 0; w < SAMPLED_CACHEWAY; w++){
        sampled_cache[set].sampled_cacheline[w].ETR--;
    }
#endif
...
REPLACEMENT_WAY_FOUND:
    ...
    if(cache_way >= SAMPLED_CACHEWAY){
        ...
        if (pc_data.count(pc_signature)){
            sampled_cache[set].sampled_cacheline[replaceable_way].ETR = pc_data[pc_signature].RDP;
        }
        else {
            sampled_cache[set].sampled_cacheline[replaceable_way].ETR = HISTORY;
        }
#endif
    }
...
//Do we have to include elapsed_time < INFINITE_RD condition here??
...
#if ETR_UPDATE
            int rd_accuracy = sampled_cache[set].sampled_cacheline[cache_way].ETR;
            pc_data[last_signature].RDPaccuracy.push_back(rd_accuracy);

            int size = pc_data[last_signature].RDPaccuracy.size();
            pc_data[last_signature].RDPaccuracy_mean = 0;
            for (int i = size-1; i > size-9; i--){
                pc_data[last_signature].RDPaccuracy_mean += pc_data[last_signature].RDPaccuracy[i]; 
            }
            pc_data[last_signature].RDPaccuracy_mean /= 8;
            pc_data[last_signature].RDPaccuracy_meanlist.push_back(pc_data[last_signature].RDPaccuracy_mean); 
#endif
        }
...
#if ETR_UPDATE
        if (pc_data.count(pc_signature)){
            sampled_cache[set].sampled_cacheline[cache_way].ETR = pc_data[pc_signature].RDP;
        }
        else {
            sampled_cache[set].sampled_cacheline[cache_way].ETR = HISTORY;
        }
#endif
    }
    ...
    if(way < LLC_WAYS) {
        if(pc_data.count(pc_signature)){
            ...
            cache.ETRerror[set][way] = pc_data[pc_signature].RDPaccuracy_mean;
        }
        else{
            cache.ETR[set][way] = 0;
            cache.ETRerror[set][way] = HISTORY;
        }
    }
...
//            ETRfile << "\t" <<"victim:" << "\t"<< way; 
   ...
   int64_t etr[LLC_WAYS];
    ...
    int variation = cache.ETRerror[set][max_etr_way];
