...
//Do we have to include elapsed_time < INFINITE_RD condition here??
        if (pc_data.count(last_signature)){
            ...
            if(abs(diff) < max(abs((1*elapsed_time)/8),(int64_t)1)) {
                rd_accuracy = 1;
            }
            else if(abs(diff) < max(abs((2*elapsed_time)/8),(int64_t)1)) {
                rd_accuracy = 2;
            }
            else if(abs(diff) < max(abs((3*elapsed_time)/8),(int64_t)1)) {
                rd_accuracy = 3;
            }
            else if(abs(diff) < max(abs((4*elapsed_time)/8),(int64_t)1)) {
                rd_accuracy = 4;
            }
            else if(abs(diff) < max(abs((5*elapsed_time)/8),(int64_t)1)) {
                rd_accuracy = 5;
            }
            else if(abs(diff) < max(abs((6*elapsed_time)/8),(int64_t)1)) {
                rd_accuracy = 6;
            }
            else if(abs(diff) < max(abs((7*elapsed_time)/8),(int64_t)1)) {
                rd_accuracy = 7;
            }
            else{
                rd_accuracy = 8;
...
//            int temp1 = (cache.ETRinit[set][way] * (temp - 1)) / 8;
//            ETRfile <<"(" << temp <<")"<<"(" << temp1 <<")";
//        } 
//    }
#endif
