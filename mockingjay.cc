...
constexpr int LOG2_LLC_SET = lg2(LLC_SETS);
constexpr int LOG2_LLC_SIZE = LOG2_LLC_SET + lg2(LLC_WAYS) + LOG2_BLOCK_SIZE;
 ...
 * current_set: an array of BLOCK, of size 16 */
 int32_t CACHE::find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t pc, uint64_t full_addr, uint32_t type)
{
