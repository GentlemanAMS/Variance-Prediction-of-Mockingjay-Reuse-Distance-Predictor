...
//Do we have to include elapsed_time < INFINITE_RD condition here??
    ...
    if(way < LLC_WAYS) {
        if(hit && type!= WRITEBACK){
            if(abs(cache.ETR[set][way]) < max(abs((1*cache.ETRinit[set][way])/8),(int64_t)1)) {
                rd_accuracy = 1;
            }
            else if(abs(cache.ETR[set][way]) < max(abs((2*cache.ETRinit[set][way])/8),(int64_t)1)) {
                rd_accuracy = 2;
            }
            else if(abs(cache.ETR[set][way]) < max(abs((3*cache.ETRinit[set][way])/8),(int64_t)1)) {
                rd_accuracy = 3;
            }
            else if(abs(cache.ETR[set][way]) < max(abs((4*cache.ETRinit[set][way])/8),(int64_t)1)) {
                rd_accuracy = 4;
            }
            else if(abs(cache.ETR[set][way]) < max(abs((5*cache.ETRinit[set][way])/8),(int64_t)1)) {
                rd_accuracy = 5;
            }
            else if(abs(cache.ETR[set][way]) < max(abs((6*cache.ETRinit[set][way])/8),(int64_t)1)) {
                rd_accuracy = 6;
            }
            else if(abs(cache.ETR[set][way]) < max(abs((7*cache.ETRinit[set][way])/8),(int64_t)1)) {
                rd_accuracy = 7;
            }
            else {
                rd_accuracy = 8;
...
//            ETRfile << "\t" <<"victim:" << "\t"<< way; 
    ...
    if (pc_data.count(cache.signature[set][max_etr_way])){
        ...
        int temp = pc_data[cache.signature[set][max_etr_way]].RDPaccuracy_mean;
        int variation = (cache.ETRinit[set][max_etr_way] * (temp - 1)) / 8;
